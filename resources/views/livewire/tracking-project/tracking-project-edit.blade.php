<div>
    <form>
        <div class="row">
            ...
            <div class="col-12 col-md-6">
                <x-component::form.prepend-input type='text' label='Project Name' name="project_name" iconName="marker"
                    :required="true" wire:model.defer='project_name' />
            </div>
            <div class="col-12 col-md-6">
    ...
    <form wire:submit.prevent="savedata">
        @foreach ($oldProject as $index => $form)
            <div class="row border mb-3 p-2 rounded">
                <div class="col-12 col-md-6">
                    <div class="form-group">
                        <label>Job Accomplish</span>
                            <span class="text-danger">*</span>
                            @error('oldProject.' . $index . '.user_deadline_accomplish_date')
                        ...
                        <div class="input-group">
                            ...
                            <input wire:model='oldProject.{{ $index }}.user_deadline_accomplish_date'
                                type="date"
                                class="form-control @error('oldProject.' . $index . '.user_deadline_accomplish_date') is-invalid @enderror">
                        </div>
                    </div>
                    {{-- <x-component::form.prepend-input
                        wire:model.defer='oldProject.{{ $index }}.user_deadline_accomplish_date' type='date'
                        label='Job Accomplish' name="user_deadline_accomplish_date" iconName="calendar"
                        :required='true' /> --}}
                </div>
                <div class="col-12 col-md-6">
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            ...
                            <input wire:model='oldProject.{{ $index }}.job' type="text"
                                class="form-control @error('oldProject.' . $index . '.job') is-invalid @enderror">
                        </div>
                    </div>
                    {{-- <x-component::form.prepend-input wire:model.defer='oldProject.{{ $index ?? $form->id }}.job'
                        type='text' label='Job' name="job" iconName="calendar" :required='true' /> --}}
                </div>
                <div class="col-12 col-md-6">
                    <x-component::form.select wire:model.defer='oldProject.{{ $index }}.user_id'
                        label='Responsible' :required='true' name='user_id'>
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}" @selected($form['user_id'] == $user->id)>
                ...
                {{-- <div class="col-12 col-md-12">
                    ...
                    <button wire:click='removeOldForm({{ $index }})'
                        class="btn btn-danger float-right" type="button" title="Remove">Delete</button>
                </div> --}}
            </div>
        @endforeach
        <x-component::button btnType="success " name="Update" wire:loading.attr='disabled' />
    </form>
    <form wire:submit.prevent="saveDynamic">
        @foreach ($dynamicForm as $index => $form)
            <div class="row border my-3 p-2 rounded">
                <div class="col-12 col-md-6">
        ...
        @endforeach
        @if (count($dynamicForm) > 0)
            <x-component::button btnType="success " name="submit" wire:loading.attr='disabled' />
            <p>Note: To update top filed click on update button.</p>
        @endif
    </form>
</div>
